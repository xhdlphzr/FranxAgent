...
FranxAI 是一个轻量级的 AI 智能体框架，让 AI 能够读取文件、执行命令、搜索网络、理解多模态内容，并通过 MCP 协议真正操控世界。**现在，它还拥有「思绪回响」般的向量知识库：所有工具说明、技能文件、乃至对话历史，都会被自动向量化并持久存储。每次提问，AI 都能通过**混合检索（向量语义 + 关键词匹配）**精准定位最相关的知识片段，注入系统提示，实现长期记忆、跨会话回忆与动态知识增强。从此，AI 不仅能「从语言到世界」，更能「让每一次回响都成为思想的养分」。**
...
- **长期记忆**：FranxAI 不再依赖 `memory.txt`，而是将完整对话历史自动保存到 `skills/memories/` 目录（每个会话一个 `.md` 文件）。下次启动时，这些历史会被自动加载到向量知识库中，AI 可以通过**混合检索**（向量语义 + 关键词匹配）回忆起之前的对话内容。
